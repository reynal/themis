...
/**
 * Utility methods for MIDI examples from java sound resources --- only
 * temporary
 */
public class MidiCommon {
		...
		for (int i = 0; i < aInfos.length; i++) {
			try {
				...
				if ((bAllowsInput && bForInput) || (bAllowsOutput && bForOutput)) {
					if (bVerbose) {
						out("" + i + "  " + (bAllowsInput ? "IN " : "   ") + (bAllowsOutput ? "OUT " : "    ")
								+ aInfos[i].getName() + ", " + aInfos[i].getVendor() + ", " + aInfos[i].getVersion()
	 ...
	 * 
	 * This method tries to return a MidiDevice.Info whose name matches the passed
	 * name. If no matching MidiDevice.Info is found, null is returned. If
	 * bForOutput is true, then only output devices are searched, otherwise only
	 * input devices.
	 * 
	 * @param strDeviceName
	 ...
	 * 
	 * @param bForOutput
	 *            If true, only output devices are considered. If false, only input
	 *            devices are considered.
	 * 
	 * @return A MidiDevice.Info object matching the passed device name or null if
	 *         none could be found.
	 * 
	 */
	 ...
	 * MidiDevice.Info whose index is specified as parameter. This index matches the
	 * number printed in the listDevicesAndExit method. If index is too small or too
	 * big, null is returned.
	 *
	 * @param index
	 *            the index of the device to be retrieved
	 * @return A MidiDevice.Info object of the specified index or null if none could
	 *         be found.
	 */
	...
	public static void main(String[] args) {
		...
		MidiDevice.Info info = getMidiDeviceInfo(1);
		out(info.toString());
