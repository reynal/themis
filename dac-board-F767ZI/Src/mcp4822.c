...
uint8_t txSpiMCP4822Buff[2]; // two byte format for the 4822 DAC


/* Private function prototypes -----------------------------------------------*/

static void dacMCP4822Write(int word12bits, int chip, int channelAB);
static void dacMCP4822AddressSelect(int chipNumber);
...
static void dacMCP4822Write(int word12bits, int chip, int channelAB){
	...
	// selection du CNA:
	dacMCP4822AddressSelect(chip);

	txSpiMCP4822Buff[0]=(((word12bits >> 8) & 0x0F)) | channelAB;
	txSpiMCP4822Buff[1]=(word12bits & 0xFF);

	// PB1 = 0 (=input ENABLE of 74LS138, which indirectly selects the corresponding circuit)
	HAL_GPIO_WritePin(DAC_CS_GPIO_Port, DAC_CS_Pin, GPIO_PIN_RESET);
	HAL_SPI_Transmit_IT(hspiDacs, txSpiMCP4822Buff, 2);
...
void dacWrite(int word12bits, Dac targetDac){
	...
	// ------- 13700 -------
		...
		case DAC_VCO_13700_FREQ:
			dacMCP4822Write(word12bits, 0,  MCP4822_CHANNEL_A_GAIN2);
			break;

	// ------- 3340A -------

		case DAC_VCO_3340A_FREQ :
			dacMCP4822Write(word12bits, 3, MCP4822_CHANNEL_A_GAIN2);
			break;

		case DAC_VCO_3340A_PWM_DUTY :
			dacMCP4822Write(word12bits, 3, MCP4822_CHANNEL_B_GAIN2);
			break;

	// ------- 3340B ------- (tout a fait a droite, "en rabe")

		case DAC_VCO_3340B_FREQ :
			dacMCP4822Write(word12bits, 7, MCP4822_CHANNEL_A_GAIN2);
			break;

		case DAC_VCO_3340B_PWM_DUTY :
			dacMCP4822Write(word12bits, 7, MCP4822_CHANNEL_B_GAIN2);
			break;

	// ------- Noise -------

		case DAC_NOISE:
			dacMCP4822Write(word12bits, 0, MCP4822_CHANNEL_B); // same DAC as 13700 (second on the left)
			break;
	...
	// ------- Mixer 1 2 3 4 (southern one, i.e., closer to the STM32 DAC board ; 1 2 3 4 go from south to north) -------
		...
		case DAC_V2140D_IN3 : // 3340B pulse
			dacMCP4822Write(word12bits, 1, MCP4822_CHANNEL_A);
			break;

		case DAC_V2140D_IN4 : // not connected yet
			dacMCP4822Write(word12bits, 1, MCP4822_CHANNEL_B);
			break;

		case DAC_V2140D_3340B_TRI_LVL:
		case DAC_V2140D_IN1 : // 3340B triangle
			dacMCP4822Write(word12bits, 2, MCP4822_CHANNEL_A);
			break;

		case DAC_V2140D_3340B_SAW_LVL:
		case DAC_V2140D_IN2 : // 3340B saw
			dacMCP4822Write(word12bits, 2, MCP4822_CHANNEL_B);
			break;
	...
	// ------- Mixer 5 6 7 8 (northern one, that is, closer to the XLR jack plug ; 5 6 7 8 go from south to north) -------
		...
		case DAC_V2140D_IN7 : // GAIN3_B on EAGLE schematic
			dacMCP4822Write(word12bits, 4, MCP4822_CHANNEL_A);
			break;

		case DAC_V2140D_VCA :
		case DAC_V2140D_IN8:  // GAIN4_B on EAGLE schematic
			dacMCP4822Write(word12bits, 4, MCP4822_CHANNEL_B_GAIN2);
			break;

		case DAC_V2140D_IN5 : // GAIN1_B on EAGLE schematic, not connected yet
			dacMCP4822Write(word12bits, 5, MCP4822_CHANNEL_A);
			break;

		case DAC_V2140D_IN6 : // GAIN2_B on EAGLE schematic, not connected yet
			dacMCP4822Write(word12bits, 5, MCP4822_CHANNEL_B);
			break;
	...
	// ------- VCF -------
		...
		case DAC_VCF_CUTOFF:
			dacMCP4822Write(word12bits, 6, MCP4822_CHANNEL_A);
			break;

		case DAC_VCF_RES :
			dacMCP4822Write(word12bits, 6, MCP4822_CHANNEL_B_GAIN2);
			break;
...
void testDacWriteSPI(){
	...
	while(1){
		...
		for(word12bits=0; word12bits < 4096; word12bits+=16){
			for(dac=0; dac < 8; dac++){
				//dacWrite(word12bits, DAC_VCO_3340_FREQ);
				dacMCP4822Write(word12bits, dac, MCP4822_CHANNEL_A);
				HAL_Delay(1);
				dacMCP4822Write(word12bits, dac, MCP4822_CHANNEL_B);
				HAL_Delay(1);
			}
